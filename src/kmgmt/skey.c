...
DISPATCH_SKEYMGMT_FN(chacha20, free);
DISPATCH_SKEYMGMT_FN(chacha20, import);
DISPATCH_SKEYMGMT_FN(chacha20, export);
DISPATCH_SKEYMGMT_FN(chacha20, generate);
DISPATCH_SKEYMGMT_FN(chacha20, get_key_id);
DISPATCH_SKEYMGMT_FN(chacha20, gen_settable_params);
DISPATCH_SKEYMGMT_FN(chacha20, imp_settable_params);

static void p11prov_chacha20_free(void *key)
{
    P11PROV_debug("chacha20 free %p", key);
    p11prov_obj_free((P11PROV_OBJ *)key);
}
                                     ...
                                     const OSSL_PARAM params[])
{
    ...
    if (p) {
        ...
        return p11prov_obj_import_secret_key(ctx, CKK_CHACHA20, key, keylen);
    }
...
static int p11prov_chacha20_export(void *keydata, int selection,
                                   OSSL_CALLBACK *param_cb, void *cbarg)
{
    P11PROV_OBJ *key = (P11PROV_OBJ *)keydata;
...
static void *p11prov_chacha20_generate(void *provctx, const OSSL_PARAM params[])
{
    ...
    /* Key length -- accepted only if it matches the fixed 256-bit size */
    p = OSSL_PARAM_locate_const(params, OSSL_SKEY_PARAM_KEY_LENGTH);
    if (p) {
        size_t len;
        ret = OSSL_PARAM_get_size_t(p, &len);
...
static const char *p11prov_chacha20_get_key_id(void *keydata)
{
    P11PROV_OBJ *key = (P11PROV_OBJ *)keydata;
...
static const OSSL_PARAM chacha20_import_params[] = {
    OSSL_PARAM_octet_string(OSSL_SKEY_PARAM_RAW_BYTES, NULL, 0),
    OSSL_PARAM_octet_ptr(OSSL_OBJECT_PARAM_REFERENCE, NULL, 0), OSSL_PARAM_END
};

static const OSSL_PARAM *p11prov_chacha20_imp_settable_params(void *provctx)
{
    return chacha20_import_params;
}

static const OSSL_PARAM chacha20_generate_params[] = {
    OSSL_PARAM_size_t(OSSL_SKEY_PARAM_KEY_LENGTH, NULL),
    OSSL_PARAM_utf8_string(P11PROV_PARAM_URI, NULL, 0),
    OSSL_PARAM_utf8_string(P11PROV_PARAM_KEY_USAGE, NULL, 0),
    OSSL_PARAM_int(P11PROV_PARAM_EPHEMERAL, NULL), OSSL_PARAM_END
...
static const OSSL_PARAM *p11prov_chacha20_gen_settable_params(void *provctx)
{
    return chacha20_generate_params;
}

const OSSL_DISPATCH p11prov_chacha20_functions[] = {
    DISPATCH_SKEYMGMT_ELEM(chacha20, FREE, free),
    DISPATCH_SKEYMGMT_ELEM(chacha20, IMPORT, import),
    DISPATCH_SKEYMGMT_ELEM(chacha20, EXPORT, export),
    DISPATCH_SKEYMGMT_ELEM(chacha20, GENERATE, generate),
    DISPATCH_SKEYMGMT_ELEM(chacha20, GET_KEY_ID, get_key_id),
    DISPATCH_SKEYMGMT_ELEM(chacha20, IMP_SETTABLE_PARAMS, imp_settable_params),
    DISPATCH_SKEYMGMT_ELEM(chacha20, GEN_SETTABLE_PARAMS, gen_settable_params),
    { 0, NULL },
};
